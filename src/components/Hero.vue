<template>
    <section class="hero text-white" style="margin-top:80px; " :style="{ backgroundImage: `url(${backgroundImg})` }" >
        <div class="w-11/12 mx-auto pt-32">        
            ...
            ">
                <p class="">
                    <span class="">{{ formatGenres }}</span> 
                </p>
                <div>
                    <span class=""> | {{ getYearFromDate }} | </span>
                    <span class="text-dark-yellow"> {{ formatTime }}</span>
                </div>
            </div>
            <div class="mt-5">
                <h2 class="text-4xl ">
...
export default defineComponent({
    ...
    props: {
        backgroundImg: {
            type: String, 
            default: () => 'https://image.tmdb.org/t/p/original/ovM06PdF3M8wvKb06i4sjW3xoww.jpg', // set a default value of an empty array
        },
        movie: {
            type: Object as () => MovieDetail | Movie, 
            default: () => ({} as MovieDetail | Movie)
        },
        genres: {
            type: Array, 
            default: () => [
    ...
    computed: {
        formatGenres(){
            return (this.genres as Array<{ id: number, name: string }>).map(genre => genre.name).join(', ');
        },
        getYearFromDate() {
            return new Date(this.movie?.release_date).getFullYear();
        },
        formatTime() {
            if ('runtime' in this.movie && this.movie.runtime) {
